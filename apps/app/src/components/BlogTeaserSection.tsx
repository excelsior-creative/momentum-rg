...
type BlogPost = { title: string; excerpt: string | null; slug: string | null; href: string; date: string | null; category: string };
  ...
  async (): Promise<BlogPost[]> => {
    try {
      ...
      if (result.docs.length > 0) {
        return result.docs.map((post: Post) => ({
          title: post.title,
...
export const BlogTeaserSection = async () => {
  ...
  const posts: BlogPost[] = livePosts.length > 0 ? livePosts : STATIC_POSTS;
